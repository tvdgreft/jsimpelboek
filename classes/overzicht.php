...
class Overzicht
{
    ...
    function Balans() : array
    {
        ...
        $balansrekeningen = $dbio->ReadRecords(array("table"=>$table_rekeningen,"filters"=>array("soort"=>"B"),"sort"=>"rekeningnummer"));
        // lees alle boekingen in lopend boekjaar
        $table_boekingen = Dbtables::boekingen['name']."_".$GLOBALS['boekhouding'];
        $boekingen = $dbio->ReadRecords(array("table"=>$table_boekingen,"filters"=>array("datum"=>$boekjaar)));  # filter alleen op jaartal
        // bedragen berekenen die aan de beginbalans toegevoegd moeten worden.
        $balans = array();
        foreach ($balansrekeningen as $p)
        {
            $beginbalans = $dbio->ReadRecords(array("table"=>$table_balans,"filters"=>array("boekjaar"=>$boekjaar-1,"rekeningnummer"=>$p->rekeningnummer)));
    ...
    function OudeBalans(int $boekjaar) : array
    {
        ...
        // Beginbalans inlezen
        $table_balans = Dbtables::balans['name']."_".$GLOBALS['boekhouding'];
        $balansposten = $dbio->ReadRecords(array("table"=>$table_balans,"filters"=>array("boekjaar"=>$boekjaar)));
        // bedragen berekenen die aan de beginbalans toegevoegd moeten worden.
        $balans = array();
        $total =$totalc = $totald = 0;
        foreach ($balansposten as $b)
        {
            $p = $dbio->ReadUniqueRecord(array("table"=>Dbtables::rekeningen['name']."_".$GLOBALS['boekhouding'],"key"=>"rekeningnummer","value"=>$b->rekeningnummer));
    ...
    function Resultaten(int $boekjaar) : array
    {
        ...
        $table_rekeningen = Dbtables::rekeningen['name']."_".$GLOBALS['boekhouding'];
        $resultaatrekeningen = $dbio->ReadRecords(array("table"=>$table_rekeningen,"filters"=>array("soort"=>"R"),"sort"=>"rekeningnummer"));
        // lees alle boekingen in lopend boekjaar
        $table_boekingen = Dbtables::boekingen['name']."_".$GLOBALS['boekhouding'];
        $boekingen = $dbio->ReadRecords(array("table"=>$table_boekingen,"filters"=>array("datum"=>$boekjaar)));  # filter alleen op jaartal
        // bedragen berekenen die aan de beginbalans toegevoegd moeten worden.
    ...
    function Begroting(int $boekjaar) : array
    {
        $dbio = new DBIO();
        // Beginbalans inlezen
        $table_begroting = Dbtables::begroting['name']."_".$GLOBALS['boekhouding'];
        $begroting = $dbio->ReadRecords(array("table"=>$table_begroting,"filters"=>array("boekjaar"=>$boekjaar)));
        // bedragen berekenen die aan de beginbalans toegevoegd moeten worden.
        $resultaat = array();
        $result =$resultc = $resultd = 0;
    ...
    function Result() : int
    {
        ...
        // lees de balansrekeningen
        $table_rekeningen = Dbtables::rekeningen['name']."_".$GLOBALS['boekhouding'];
        $balansrekeningen = $dbio->ReadRecords(array("table"=>$table_rekeningen,"filters"=>array("soort"=>"B")));
        // lees alle boekingen in lopend boekjaar
        $table_boekingen = Dbtables::boekingen['name']."_".$GLOBALS['boekhouding'];
        $boekingen = $dbio->ReadRecords(array("table"=>$table_boekingen,"filters"=>array("datum"=>$boekjaar)));  # filter alleen op jaartal
        $result=0;
